...
Core parity: ``LocalTuyaHumidifier`` mirrors ``homeassistant/components/tuya/humidifier.py``
(``TuyaHumidifierEntity``).

SYNC CHECKLIST (when the core component is updated):
  1. Diff ``homeassistant/components/tuya/humidifier.py`` against this file.
  ...
  3. Keep our deliberate deltas (they are intentional):
     ...
     - construction: ``__init__(device, config_entry, dp_id, description=None)``
       resolves wrappers by dpcode via ``get_humidifier_definition``; the manual
     ...
     - the mode ``DictSelector`` (``_available_modes``) conversion lives in a
       wrapper decorator, not in the entity.
"""
