...
Core parity: ``LocalTuyaCover`` mirrors ``homeassistant/components/tuya/cover.py``
(``TuyaCoverEntity``).

SYNC CHECKLIST (when the core component is updated):
  1. Diff ``homeassistant/components/tuya/cover.py`` against this file.
  ...
  3. Keep our deliberate deltas (they are intentional):
     ...
     - construction: ``__init__(device, config_entry, dp_id, description=None)``
       resolves the set-position wrapper by dpcode via ``get_cover_definition``;
     ...
     - no tilt support; the movement state machine (``status_updated``) and
       timed-math / inversion live in the wrapper decorators.
"""
