...
Core parity: ``LocalTuyaClimate`` mirrors ``homeassistant/components/tuya/climate.py``
(``TuyaClimateEntity``).

SYNC CHECKLIST (when the core component is updated):
  1. Diff ``homeassistant/components/tuya/climate.py`` against this file.
  ...
  3. Keep our deliberate deltas (they are intentional):
     ...
     - construction: ``__init__(device, config_entry, dp_id, description=None)``
       resolves wrappers by dpcode via ``get_climate_definition``; the manual
     ...
     - unit conversion, the ``_hvac_switch_dp`` switch-only fallback, and the
       temp/preset/swing conversions live in the wrapper decorators, not here.
"""
